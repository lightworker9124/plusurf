...
function ajax_post(formid, showid, checkredir, redirurl)
{
	newform = '#'+formid;
	newshow = '#'+showid;
	$(newshow).html('<img src="'+app_theme+'/assets/img/loading.gif" />');
...
function countdown(id, value)
{
	...
	var counter = setInterval(function(){
		var currentnumber = document.getElementById(id).innerHTML-1;
		if(currentnumber<0)
		{
	...
	setTimeout(function(){ window.clearInterval(counter); },Math.floor(value*1000));
}

function clear_session(id, urlpost, redirect)
{
	if (urlpost != '') {
		$.ajax({
			url : urlpost,

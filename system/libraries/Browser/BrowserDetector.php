...
class BrowserDetector implements DetectorInterface
{
    ...
    {
        if (stripos($userAgent->getUserAgentString(), 'Edge') !== false) {
            $aresult = explode('/', stristr($userAgent->getUserAgentString(), 'Edge'));
            $aversion = explode(' ', $aresult[1]);
            $browser->setVersion($aversion[0]);
            $browser->setName($browser::EDGE);
            return true;
        }
		
        else if (stripos($userAgent->getUserAgentString(), 'Chrome') !== false) {
